...
const [sanityorders, setSanityorders] = useState([])
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      let a = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/sanitymyorders`,{
      method: 'POST', // or 'PUT'
      headers: {
      ...
      body: JSON.stringify({token :JSON.parse(localStorage.getItem('sanityuserinfo')).token}),
    }
  );
            ...
            <div className="py-2 inline-block min-w-full sm:px-6 lg:px-8">
              <div className="overflow-hidden">
                <table className="min-w-full">
                  ...
                  <tbody>
                    {Object.keys(sanityorders).map((od) => (
                     <tr key={od} className="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
                     <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{sanityorders[od].oid}</td>
                     ...
                     <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                       <Link href={'/sanitypayorder?_id=' + sanityorders[od]._id}><a>Details</a></Link>
                     </td>
                   </tr>
                    ))}
                ...
                <tr key={s} className="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
                      ...
                      <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                        <Link href={'/sanitypayorder?_id=' + sanityorders._id}><a>Details</a></Link>
                      </td>
                    </tr>
                      )
})} */}
