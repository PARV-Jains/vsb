...
const Slug = ({
  ...
  //     setSize(sanityproductssvar.size);
  //   }
  // }, [router.query,error]);
  ...
  const refreshVariant = (newSize) => {
    // let url = `${ process.env.NEXT_PUBLIC_HOST}/product/sanitySlug/${name.replace(/\s/g, "").toLowerCase()}-${newSize}`;
    let url = `${process.env.NEXT_PUBLIC_HOST}/product/sanitySlug/${name
      .replace(/\s/g, '')
      .toLowerCase()}`;
    // let url = `${process.env.NEXT_PUBLIC_HOST}/product/sanitySlug/${[newSize]['slug']}`;
    router.push(url);
  };
...
export const getStaticProps = async ({ params: { slug } }) => {
  ...
  // sanityproductssvar = product
  // sanityvaritem = products
  // let sanityvariants =  await client.fetch({
  //   name: sanityproductssvar.name,
  //   category: sanityproductssvar.category,
  // })
  ...
  //   console.log(sanityproductssvar);
