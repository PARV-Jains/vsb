...
const Namkeens = ({ products,sanityproductss}) => {
  ...
  //     try {
  //       const sanityproductss = await client.fetch(`*[type == "sanityproduct"]`);
  //       setState({ sanityproductss, loading: false  });
  //     } catch (err) {
            ...
            {Object.keys(products).map((item) => {
              {
                Object.keys(products).length === 0 && (
                  <p>
                    soory , saari namkeen out of stock hai .naya stock jald
                    aata hi hoga . stay tuned{' '}
                  </p>
                );
              }
              return (
                ...
                >
                  <div className="lg:w-1/5 md:w-1/2 p-4 w-full cursor-pointer  shadow-lg m-5">
                    ...
                    <div className="mt-4 text-center md:text-left">
                      ...
                      <div className="mt-1">
                        {products[item].size.includes('250g') && (
                          <span className="border border-gray-300 px-1 mx-1">
                            250g{' '}
                          </span>
                        )}
                        {products[item].size.includes('500g') && (
                          <span className="border border-gray-300 px-1 mx-1">
                            500g{' '}
                          </span>
                        )}
                        {products[item].size.includes('1kg') && (
                          <span className="border border-gray-300 px-1 mx-1">
                            1kg{' '}
                          </span>
                        )}
...
export async function getServerSideProps(context) {
  ...
  const productquery = '*[_type == "sanityproduct"&& category == "namkeen"]';
